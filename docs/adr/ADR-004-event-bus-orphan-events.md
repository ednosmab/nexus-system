...
The Nexus event bus (`src/event-bus.ts`) declares 33 event types in `NexusEventType`. Of these:
...
### Remove (never needed, remove from NexusEventType + all publish calls)
