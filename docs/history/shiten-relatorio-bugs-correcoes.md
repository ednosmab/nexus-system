# Shitenno-go — Relatório de Execução E2E + Correções

> Gerado após instalar, buildar, testar e executar o pacote `shitenno-go`
...
## 🐛 BUG 1 — RESOLVIDO: Regex corrompe `shiten-profile/*.config.ts`
...
Em `src/__tests__/scaffolder.test.ts`, novo bloco `describe("generated shiten-profile/*.config.ts content")` com 5 testes que leem o **conteúdo real** do arquivo gerado (não só verificam se ele existe):

```ts
import { mkdtempSync, rmSync, mkdirSync, existsSync, readFileSync } from "node:fs";
// ... (adicionar readFileSync ao import existente)

describe("generated shiten-profile/*.config.ts content", () => {
  ...
  function readGeneratedProfile(dir: string): string {
    ...
    const profilePath = join(dir, "shiten-profile", `${projectName}.config.ts`);
...
## 🐛 BUG 2 — ABERTO: `shiten run` ignora o lifecycle gate que `shiten evolve` respeita
...
No mesmo estado do projeto (`assessed`), chamar `shiten evolve` diretamente é bloqueado pela state machine, mas o estágio `evolve` dentro de `shiten run` executa sem checagem.
...
if (!checkLifecycleGate("evolve", ctx.projectRoot, ctx.shitenDir, true)) {
...
**Opção B — Documentar que `run` é uma "via expressa" intencional**, que avança o estado conforme necessário em vez de bloquear (mais alinhado com "Shiten propõe, humano decide": rodar `run` é uma ação humana explícita, então pode ser tratado como consentimento implícito para rodar tudo). Se for essa a intenção, adicionar um comentário explícito no código justificando, para não parecer um descuido da próxima vez que alguém ler.
...
## 🐛 BUG 3 — ABERTO: `shiten report` mostra a mesma dimensão como força E fraqueza
...
Isso é especialmente importante porque **todo projeto novo começa com dimensões empatadas** — não é um edge case raro, é o estado inicial padrão de qualquer adoção do Shiten.
...
## ⚠️ NÃO-BUG, mas observação relevante: `shiten init` é 100% interativo, sem flag não-interativa

`shiten init` faz 9 blocos de perguntas via `inquirer` (tipos `input`, `checkbox`, `list`, `confirm`), sem nenhuma flag tipo `--yes` ou `--config <file>` para rodar de forma não-interativa/scriptável. Isso é aceitável para uso manual direto pelo Tech Lead, mas significa que **não é possível automatizar a inicialização do Shiten a partir de outro script ou CI** sem ou (a) adicionar essa flag, ou (b) chamar `scaffoldShitennoGo` programaticamente (que é o que usei para testar nesta sessão, contornando o prompt).

Se o objetivo for permitir, por exemplo, que um template de projeto já venha com Shiten pré-configurado automaticamente, vale considerar adicionar:

```bash
shiten init --answers-file ./shiten-answers.json
```
...
- `scaffoldShitennoGo` (pós-correção) gera 24 arquivos / 10 pastas corretamente
- `shiten status` produz health check + complexity breakdown por área, granular e legível
- `shiten assess` recalcula o perfil de maturidade e avança o estado da máquina corretamente
- `shiten doctor` dá a saída mais útil de todas — risco, melhoria, e uma seção "Learn" explicando conceito (ADR vs Skill) de forma natural, sem parecer enfeite
- `shiten report` reusa genuinamente as 7 dimensões do `feedback/TEMPLATE.md` original do course-platform — a herança conceitual é real, não só similar de nome
