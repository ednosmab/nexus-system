...
| Auto-backlog feature | Alto | shugo audit --auto-backlog — detecta gaps e escreve no BACKLOG.md (606 testes) |
...
| AUDIT-CLEANUP-03 | Baixo | Removidos 4 casts as ShitennoEventType em pipeline.ts — 2026-07-05 |
...
| DESOPLAMENTO A.1-A.4 | Critico | Desacoplamento de opencode.json — shared.ts, shitenno-state-machine.ts, analyser.ts, capability-engine.ts — 2026-07-07 |
...
| DESOPLAMENTO B.3 | Medio | Sync restaurado em bin/shugo.ts, gate valido — 2026-07-07 |
...
| 3.6 | Baixo | shugo dashboard --live (--live <seconds>) — 2026-07-08 |
...
| **Resolucao parcial** | (2026-07-01) `shugo init` agora re-analisa complexidade quando projeto ja inicializado. `shugo assess` mostra proximo passo claro com `shugo upgrade --accept-recommended`. |
...
| **Arquivos** | README.md, docs/AGENTS.md, Shitenno_GUIDE.md, CONCEPTUAL_MODEL.md, SYSTEM_MAP.md, CHANGELOG.md |
...
| **Severidade** | Alto |
| **Owner** | Edson |
| **Resolucao** | Adicionado --session-id ao shugo feedback (2026-06-30) |
...
| **Severidade** | Medio |
| **Owner** | Edson |
| **Resolucao** | Corrigido de .shugo para shitenno/ (2026-06-30) |
| **Arquivo** | `src/commands/briefing.ts` |
...
### 1.8 Validar shitennoDir antes de operacoes
...
| **Severidade** | Medio |
| **Owner** | Edson |
| **Resolucao** | Adicionado existsSync(shitennoDir) no guardNotInitialized() (2026-06-30) |
...
| **Correcao** | Criar `shugo detect --approve RULE-001` e `shugo detect --reject RULE-001`. Persistir decisoes em `shitenno/governance/rules-decisions.json`. |
...
### 2.3 Shugo update — comando de actualizacao com change detection
...
| **Prioridade** | P2 |
| **Data** | 2026-07-04 |
| **Fonte** | Sessao 2026-07-04 — nao existe `shugo update`, templates congelados no install time |
| **Arquivos** | `src/commands/update.ts` (novo), `src/manifest.ts` (novo), `src/commands/init.ts`, `src/commands/upgrade.ts`, `bin/shugo.ts` |
| **Descricao** | Nao existe `shugo update`. O `shugo upgrade` so adiciona capabilities novas, nunca actualiza existentes. Templates estao congelados no install time. Shitenno-system precisa saber que houve mudanças no shitenno-cli. |
| **Resolucao** | (1) Criado `src/manifest.ts` para rastrear hashes de templates e cliVersion. (2) Integrado manifest em init e upgrade. (3) Criado comando `shugo update` com suporte a `--apply`, `--dry-run`, `--backup` e `--force` (2026-07-05). |
...
### 2.3b shugo bench --compare historico
...
| **Correcao** | Salvar resultado em `shitenno/reports/bench-YYYY-MM-DD.json`. Adicionar `--compare` que mostra tendencia. |
...
| **Owner** | Edson |
| **Data** | 2026-07-03 |
| **Fonte** | Briefing de onboarding (briefing-onboarding-shitenno.md, criterio de aceite #5) |
...
| **Descricao** | Validar que o reescrita de onboarding (README + dashboard discover/use) atinge o criterio dos 5 minutos: pessoa sem contexto consegue correr `shugo init` e entender o output sem perguntar nada. Teste manual com participante real. |
| **Correcao** | (1) Seguir o protocolo em `docs/tests/onboarding-5min-test.md`. (2) Participante deve ser alguem que nunca usou Shugo. (3) Cronometrar tempo ate primeiro comando e tempo ate compreensao. (4) Se falhar, identificar a pagina/secao que causou confusao e corrigir. (5) Repetir apos correcao. |
| **Criterio de aceite** | Todos os 4 criterios do teste passam: encontrou install em ≤60s, rodou init em ≤3min, explicou o que Shugo faz em ≤5min, nao pediu ajuda. |
...
| ~~3.6~~ | ~~shugo dashboard --live~~ | ~~Baixo~~ | ~~Done: --live <seconds> opção existe em dashboard.tsx~~ |
...
| ~~3.10~~ | ~~shugo --quiet / --no-color~~ | ~~Baixo~~ | ~~Done: --quiet (SHITENNO_QUIET) + --no-color (chalk.level=0) em bin/shugo.ts + logger.ts — 2026-07-12~~ |
...
| ~~3.14~~ | ~~Changelog automatico~~ | ~~Baixo~~ | ~~Done: shitenno/scripts/generate-changelog.ts — conventional commits~~ |
...
| ~~3.19~~ | ~~shugo detect --approve/--reject~~ | ~~Medio~~ | ~~Done: --approve e --reject implementados em src/commands/detect.ts~~ |
| ~~3.20~~ | ~~shugo feedback --outcome failure auto-link~~ | ~~Baixo~~ | ~~Done: sugere failure hotspots quando --areas nao fornecido~~ |
...
| ~~3.25~~ | ~~shugo bench --save / --load~~ | ~~Baixo~~ | ~~Done: loadBenchHistory, saveBenchResult + --compare em bench.ts~~ |
...
| ~~3.31~~ | ~~shugo detect --format markdown~~ | ~~Baixo~~ | ~~Done: --format text/json/markdown em detect.ts~~ |
...
| 3.36 | Decidir nome do produto | Baixo | Shugo Gaude, Prism, Codex, ou outro — decisao estrategica pendente |
...
|---|---|
| **Status** | Done |
| **Resolucao** | mcp-server.ts com 3 tools (getBriefing, getRiskMap, getRules). mcp-install.ts para filesystem server. Comando shugo mcp registado em bin/shugo.ts. |
| **Severidade** | Alto |
| **Owner** | Edson |
| **Descricao** | Servidor MCP (Model Context Protocol) para agentes IA consumirem contexto do Shugo. Ferramentas: getBriefing, getRiskMap, getRules. |
| **Correcao** | Criar `src/mcp-server.ts` com ferramentas MCP. Publicar como `@shugo/mcp`. |
...
| **Correcao** | Criar extensao VS Code (Cursor e compativel) que le `.shugo/BRIEFING.md` e exibe em panel. |
...
| **Resolucao** | shugo hooks --install/--uninstall implementado em src/commands/hooks.ts 2014 2026-07-11 |
...
| **Correcao** | Adicionar `--webhook <url>` ao `shugo briefing`. Enviar JSON via POST apos gerar. |
...
| **Correcao** | Adicionar `shugo serve` que inicia servidor HTTP com endpoints: GET /briefing, GET /risk-map, GET /rules. |
...
| **Severidade** | Alto |
| **Owner** | Edson |
| **Resolucao** | Implementado feedback-engine.ts com calibragem de tom por perfil (2026-07-01). Comando `shugo feedback --personalized` gera feedback duplo (agente + usuario) com 3 tons (mentor/peer/relatorio) baseado no perfil do usuario. Comando `shugo profile` para configurar perfil. Perguntas de perfil no `shugo init`. Funcao `inferProfile()` infere perfil do comportamento. Atualizacao automatica do perfil apos cada sessao. 19 testes novos. |
...
| **Severidade** | Medio |
| **Owner** | Edson |
| **Descricao** | `shugo tutorial` — guided tour interativo que mostra cada comando com exemplos reais. |
...
| **Correcao** | Criar `examples/` com 3 diretorios. `shugo init --template web-app` para usar. |
...
| **Severidade** | Baixo |
| **Owner** | Edson |
| **Descricao** | Como migrar de outros tools (ESLint, Prettier, SonarQube) para Shugo. |
...
| **Severidade** | Baixo |
| **Owner** | Edson |
| **Descricao** | Uso como biblioteca Node.js, nao CLI. `import { generateBriefing } from 'shitenno'`. |
| **Correcao** | Separar core de CLI. Criar `shitenno/core` export. |
...
| **Correcao** | Dashboard `shugo analytics` que le `usage.jsonl` e gera graficos. |
...
> **Gerado por:** shugo assess + shugo doctor + shugo audit + shugo status + análise manual
...
| **Owner** | Edson |
| **Data** | 2026-06-30 |
| **Fonte** | shugo assess --json (dimension: governance = 0) |
| **Modulos** | src/ (global) |
...
| **Prioridade** | P1 |
| **Owner** | Edson |
| **Resolucao** | context_buffer.yaml movido para core capability, sempre criado no shugo init — 2026-07-08 |
...
| **Fonte** | Analise manual |
| **Modulos** | src/ (global) |
| **Descricao** | God modules (feedback-loops.ts 396 linhas, state-manager.ts 438 linhas). Sem dependency injection. Interface Segregation violada (ShitennoState com 60+ campos). |
...
| **Correcao** | Executar `shugo audit` para popular knowledge graph automaticamente. |
...
Auto-backlog:  shugo audit --auto-backlog (detect gaps → BACKLOG.md)
