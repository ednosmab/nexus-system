# Engineering

> Development process, contributing guidelines, and governance rules.

## Purpose

This directory contains process documentation for contributing to and maintaining the Shugo CLI project. It defines how we work, not what we build.

## Audience

- Contributors submitting PRs
- Maintainers releasing versions
- AI agents working on the codebase

## Documents

| Document | Purpose | Status |
|----------|---------|--------|
| [Backlog](../BACKLOG.md) | Active work tracker with P0-P3 priorities | Active |
| [Documentation Governance](./DOCUMENTATION_GOVERNANCE.md) | Rules for documentation maintenance | Planned |
| [Contributing Guide](./CONTRIBUTING.md) | How to contribute to Shugo | Planned |
| [Workflow](./WORKFLOW.md) | Development workflow | Planned |
| [AI Agent Rules](./AGENTS.md) | Rules for AI agents on this repo | Planned |
| [Development Guide](./DEVELOPMENT.md) | Development environment setup | Planned |
| [Coding Standards](./CODING_STANDARDS.md) | Code style and quality rules | Planned |
| [Release Process](./RELEASE_PROCESS.md) | How releases are managed | Planned |

## Related Documentation

- [Architecture](../architecture/) — System design
- [Implementation](../implementation/) — Build history

---

*Last updated: 2026-06-29*
