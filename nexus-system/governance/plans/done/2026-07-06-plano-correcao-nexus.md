# Plano de Correção — Nexus System (sessão 2026-07-06)

**Origem:** auditoria de continuação sobre o zip `nexus-system-main`, feita rodando o código de verdade (build, testes, `nexus init`/`audit`/`clean` em projetos reais), não apenas lendo diffs.
...
- Dois bugs novos desta sessão (itens 1 e 2 abaixo) só apareceram porque testei `nexus init` em projeto limpo e depois rodei `audit` no próprio repo — leitura de código não teria pego nenhum dos dois.
...
```ts
{ src: "nexus-system/docs/BACKLOG.md", dest: "docs/BACKLOG.md" },
```
O `src` real do template é `docs/BACKLOG.md` (sem prefixo `nexus-system/`), e o `dest` correto (o path que `rule-engine.ts` de fato lê via `join(context.nexusDir, "docs", "BACKLOG.md")`) é `nexus-system/docs/BACKLOG.md`. Como o `scaffolder.ts` faz `if (!existsSync(srcPath)) continue;` (skip silencioso), o arquivo nunca era copiado — em nenhum projeto, mesmo sob a capability `core` (sempre instalada).
...
**Verificação a repetir:** `nexus init` num diretório limpo → `find . -iname BACKLOG.md` deve achar `nexus-system/docs/BACKLOG.md`.
...
**Verificação a repetir:** rodar `nexus audit --level full --json --no-cache` no próprio repo, checar que não há `console_secret`/`xss_risk` apontando pra `src/audit/`.
...
**O que aconteceu:** em algum momento o comando `sync` foi removido inteiramente do CLI (não está mais em `bin/nexus.ts`, não aparece em `--help`). Mas ficaram 2 resíduos:
...
|---|---|---|---|
| `log_event` (RULE-001, 006, 008, 009) | `nexus-system/docs/history/` | `metrics` | ❌ Não |
| `create_reminder` (RULE-006, 007, 010) | `governance/context/context_buffer.yaml` | `governance` | ❌ Não |
| `update_backlog` (RULE-005, 010) | `nexus-system/docs/BACKLOG.md` | `core` (ver item 1) | ✅ Sim (após fix) |
...
**Eventos declarados em `NexusEventType` (`src/event-bus.ts`) sem nenhum `.publish(...)` em lugar nenhum do código:**
- `score.calculated`
- `debt.detected`
- `capability.unlocked`
...
- **Remover o evento** de `NexusEventType` e de qualquer regra/mapeamento que dependa dele, se não fizer mais sentido.
...
- Gate de lifecycle do comando `clean` — testei rodando `nexus clean` num projeto em estado `assessed` (deveria exigir `governed`) → bloqueou corretamente.
...
- Fase 13/14 (doc-sync-hook removido do bootstrap one-shot, Rule Engine com 10 regras reais persistidas) — confirmado por execução real (`nexus init` + inspeção de `governance/rules/*.json`).
