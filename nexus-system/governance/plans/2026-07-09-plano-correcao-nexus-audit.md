# Plano de Correção — Nexus Audit (validação profunda + hardening)
...
O `nexus audit` tem uma arquitetura sofisticada (TS Compiler API real para taint analysis, 45 detectores,
4 níveis de profundidade) mas tem **3 classes de problema** que juntas fazem o sistema reportar segurança
falsa em código real:

1. **Postura errada por padrão** — os 12 detectores de segurança e o taint analyzer só rodam em `--level full`/`code-review`. O comando óbvio (`nexus audit`) roda em `standard` e nunca vê SQL injection, XSS, secrets, etc.
...
MD5, HTTP inseguro, path traversal, prototype pollution, ReDoS, deserialização — `nexus audit` (padrão) não
reportou **nenhum** desses. `nexus audit --level full` reportou 9 de 10.
...
`yarn.lock` ou `bun.lockb`, apesar de o *próprio* Nexus usar pnpm (`detectMissingLockFile`, linha 965, já
...
`uuid` (CVSS 7.5, CWE-787/1285, "Missing buffer bounds check"). `nexus audit --level full` e `--level
...
| Categoria OWASP 2025 | Cobertura atual do Nexus | Ação |
|---|---|---|
...
## Parte 3 — O "bloqueio" da pasta `nexus-system` (single source of truth do nome)

**Achado:** a string literal `"nexus-system"` está hardcoded em **51 ocorrências, 29 arquivos**
(`src/utils.ts`, `src/shared.ts`, `src/scaffolder.ts`, `bin/nexus.ts`, e mais 25). Não existe hoje nenhuma
constante única. Se o produto for renomeado, é find-and-replace manual em 29 arquivos, com risco real de
esquecer um lugar (e o próprio `nexus-system/` é usado como sentinela de detecção de projeto — ver `utils.ts`
...
```ts
// src/constants.ts
/** Nome do diretório de metadados do Nexus. Única fonte de verdade — nunca hardcode a string. */
export const NEXUS_DIR_NAME = "nexus-system";
```
Depois, substituir as 51 ocorrências por `NEXUS_DIR_NAME` (a maioria é `join(projectRoot, "nexus-system")` →
`join(projectRoot, NEXUS_DIR_NAME)`). É um refactor mecânico, mas tem que ser feito com testes rodando a
...
Depois da Camada 1, adicionar uma regra de lint que **proíbe** a string literal `"nexus-system"` fora de
...
// eslint.config.js — adicionar regra
{
  ...
  rules: {
    "no-restricted-syntax": ["error", {
      ...
      message: "Use NEXUS_DIR_NAME de src/constants.ts em vez de hardcodear 'nexus-system'.",
    }],
  },
},
```
...
**Se quiser ir além:** tornar o próprio nome configurável via `nexus-system/config.json` (ex.: campo
...
2. **Cache do resultado de `npm/pnpm/yarn audit`**: esses comandos batem em rede; hoje `--level full` já
   ...
   `nexus audit` chamado no mesmo dia.
...
4. **Modo "explicar por que 1/100"**: já que os detectores de governança auditam a própria pasta
   `nexus-system/` (não o código do usuário), adicionar uma seção no output do audit que separa
   claramente "saúde da sua documentação Nexus" de "saúde do seu código" — hoje os dois número aparecem
...
Fase 4 (arquitetura)
  Parte 3: NEXUS_DIR_NAME + regra de lint de bloqueio
...
(vulnerable.ts, taint-test.ts, taint-test2/3.ts) com `nexus audit` no nível padrão e confirmar que
