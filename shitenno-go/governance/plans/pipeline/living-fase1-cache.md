# Pipeline: Shiten Living — Fase 1 (Cache Cross-Process)
...
1. Correr `shiten status` — deve responder sem erro e mostrar health score
2. Correr `shiten doctor` — deve mostrar recomendações
3. Correr `shiten audit` — deve listar assets do projecto
4. Aguardar 70 segundos (cache expira após 60s)
5. Correr `shiten status` novamente — deve usar cache do disco (mais rápido)
6. Modificar um ficheiro em `governance/` (ex: adicionar linha a um .md)
7. Correr `shiten status` — deve recalcular (cache obsoleto detectado)
8. Correr `shiten status --force-refresh` — deve recalcular sempre, ignorando cache
...
| C5 | shiten status funciona | Correr `shiten status` | Output sem erro, health score visível | pendente |
| C6 | shiten doctor funciona | Correr `shiten doctor` | Output sem erro, recomendações visíveis | pendente |
...
npx vitest run src/__tests__/engineering-state-access.test.ts \
  ...
  && shiten pipeline notify living-fase1-cache --event "phase_complete"
```
