# Pipeline: Shiten Living — Fase 3 (Daemon Opt-In)

**Status:** Pending
**Date:** 2026-07-11
**Plan:** 2026-07-11-shiten-living-plano-v2-3fases
**Phase:** 3
**Owner:** ambos
**Execucoes_humano:** 0
**Execucoes_agente:** 0
...
1. Correr `shiten daemon start` — daemon deve arrancar e ficar activo
2. Correr `shiten daemon status` — deve mostrar PID e estado "running"
3. Correr `shiten status` — deve responder via socket IPC (mais rápido)
4. Correr `shiten daemon stop` — daemon deve encerrar limpo
5. Verificar que ficheiros temporários (PID, socket) foram removidos
6. Correr `shiten daemon start` 3x seguidas com crash forçado — circuit breaker deve desistir
7. Correr `shiten daemon start` com `SHITEN_NO_DAEMON=1` — não deve arrancar daemon
8. Correr `shiten daemon start` com `CI=true` — não deve arrancar daemon
...
4. `stat -c %a shitenno-go/daemon/daemon.sock` — confirmar permissão 0600
5. `grep -n "chmod.*0600" src/daemon.ts` — confirmar chmod no código
6. `grep -n "SHITEN_NO_DAEMON" src/daemon-client.ts` — confirmar detecção de env var
...
|---|---|---|---|---|
| C1 | Daemon arranca | `shiten daemon start` | PID gravado, estado "running" | pendente |
| C2 | Daemon responde | `shiten daemon status` | Output com PID e uptime | pendente |
...
| C6 | SHITEN_NO_DAEMON=1 | Correr com env var | Nenhum daemon spawnado | pendente |
...
npx vitest run src/__tests__/daemon.test.ts src/__tests__/daemon-client.test.ts \
  ...
  && shiten pipeline notify living-fase3-daemon --event "phase_complete"
```
