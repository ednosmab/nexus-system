...
const SHITEN = resolve(ROOT, "shitenno-go");
const DOCS = resolve(SHITEN, "docs");
const SRC = resolve(ROOT, "src", "commands");
const SRC_TS = resolve(ROOT, "src");
const README = resolve(ROOT, "README.md");
const CHANGELOG = resolve(ROOT, "CHANGELOG.md");
const PACKAGE_JSON = resolve(ROOT, "package.json");
const SYSTEM_MAP = resolve(SHITEN, "governance", "SYSTEM_MAP.md");
...
function checkDocumentedDirectories() {
  ...
  for (const dir of documentedDirs) {
    const fullPath = resolve(SHITEN, dir);
...
function checkUndocumentedDirectories() {
  ...
  const shitenDirs = readdirSync(SHITEN).filter((f) => {
    try {
      return statSync(join(SHITEN, f)).isDirectory();
    } catch {
      return false;
    }
  });
...
function checkCLICommands() {
  ...
  const documented = implemented.filter((cmd) =>
    guide.includes(`shiten ${cmd}`)
  );
  const undocumented = implemented.filter(
    (cmd) => !guide.includes(`shiten ${cmd}`)
  );
  ...
  if (undocumented.length > 0) {
    for (const cmd of undocumented) {
      warn(`Command implemented but not documented: shiten ${cmd}`, undefined, false);
    }
  }
...
function checkSystemMap() {
  ...
  if (DRY_RUN) {
    ...
    error("SYSTEM_MAP.md directory tree is outdated", "shitenno-go/governance/SYSTEM_MAP.md", true);
  }
}
...
function checkScriptReferences() {
  ...
  for (const script of uniqueScripts) {
    // Skip if it's a shiten command (not an npm script)
