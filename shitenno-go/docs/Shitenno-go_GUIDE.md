# Shitenno-go — Guia Completo do Sistema de Governança

> **Nome público:** Shitenno-go
> **Ficheiro guia:** `shitenno-go/docs/Shitenno-go_GUIDE.md`
> **Versão:** 3.0
> **Data:** 2026-07-02

---

## 1. O que é o Shitenno-go

O **Shitenno-go** é um framework de governança para desenvolvimento de software assistido por inteligência artificial. Define **como** uma equipa (humana + agentes IA) deve ler, compreender, modificar e validar código num repositório complexo — de forma segura, rastreável e consistente.
...
│  shitenno-go/          ← Framework de governança           │
...
└─────────────────────────────────────────────────────────────┘
                          ▲
                          │ shiten init / shiten upgrade
                          │
┌─────────────────────────────────────────────────────────────┐
│                      SHITEN CLI (FERRAMENTA)                  │
│                                                             │
│  shiten init          ← Instala framework no projecto        │
│  shiten status        ← Verifica saúde da governança         │
│  shiten upgrade       ← Eleva capacidades (knowledge, etc.)  │
│  shiten validate      ← Valida conformidade                 │
│  shiten sync          ← Sincroniza templates                 │
└─────────────────────────────────────────────────────────────┘
                          ▲
                          │ templates/base/
                          │
...
O Shitenno-go usa uma abordagem modular baseada em **capacidades**. O `shiten init` instala a base, e `shiten upgrade --capability <name>` adiciona capacidades conforme necessário.
...
1. **`shiten init`** — Instala base completa (knowledge + architecture + governance)
2. **`shiten upgrade --capability <name>`** — Adiciona capacidade específica
3. **`shiten upgrade --accept-recommended`** — Instala todas as recomendadas pelo perfil
...
| `operacao_no_shiten.md` | Meta-skill | Como agir dentro da estrutura de pastas do Shiten |
...
- **Meta-skill (1):** `operacao_no_shiten.md` — descreve como operar dentro da estrutura de pastas

---

## 4. Os 4 Papéis de Agente IA

Cada agente IA num projecto Shiten tem um contrato definido:
...
O Shitenno-go calcula complexidade do projecto usando dois eixos:
...
│   ├── Shitenno-go_GUIDE.md        ← Este ficheiro
...
├── feedback/                        ← Feedback do Shiten CLI
│   ├── summary.json
...
O perfil do projecto é definido em `maturity-profile.json` na raiz do `shitenno-go/`:
...
```json
{
  ...
  "historyPath": "shitenno-go/docs/history",
  "feedbackPath": "shitenno-go/docs/feedback",
  "violationKeywords": ["erro", "bug", "corrigi", "falhou", "rollback"],
  "highComplexityThreshold": 7
}
```

- **Áreas** detectadas automaticamente por `shiten init` (via `detectAreas()`)
...
| Termo | Definição |
|---|---|
| **Shitenno-go** | Framework de governança para dev assistido por IA |
...
| Comando | Função |
|---|---|
| `shiten init` | Instala framework no projecto (detecta stack, gera profile) |
| `shiten init --answers-file <path>` | Instala com respostas pré-definidas (JSON, sem prompts interactivos) |
| `shiten status` | Verifica saúde da governança (auto-detecta projecto) |
| `shiten upgrade --capability <name>` | Adiciona uma capacidade (knowledge, architecture, governance, ai, quality, metrics, operations, compliance) |
| `shiten upgrade --list` | Mostra capacidades disponíveis e seu estado |
| `shiten upgrade --accept-recommended` | Instala todas as capacidades recomendadas pelo perfil |
| `shiten validate` | Valida conformidade do projecto |
| `shiten detect` | Detecta padrões no histórico de commits |
| `shiten audit` | Auto-avaliação: regras mortas, hotspots de violação |
| `shiten evolve` | Recomendações adaptativas baseadas na maturidade |
| `shiten run` | Pipeline completo de 5 estágios |
| `shiten assess` | Re-avaliação do perfil de maturidade |
| `shiten doctor` | Diagnósticos de saúde do sistema |
| `shiten report` | Gera relatórios |
| `shiten clean` | Limpa cache e ficheiros temporários |
| `shiten bench` | Benchmark de performance do sistema |
| `shiten briefing` | Gera briefing pré-sessão (output markdown ou JSON) |
| `shiten console` | Consola interactiva para debug |
| `shiten dashboard` | Dashboard de métricas e estado |
| `shiten decide` | Motor de decisões assistido por IA |
| `shiten digest` | Digest diário de actividade |
| `shiten feedback` | Gestão de feedback de sessões |
| `shiten goal` | Gestão de objectivos e roadmap |
| `shiten plan` | Geração de planos atómicos |
| `shiten policy` | Gestão de políticas operacionais |
| `shiten profile` | Gestão do perfil do projecto |
| `shiten shell-init` | Configuração de shell (completions, aliases) |
| `shiten act` | Execução de acções atómicas |
| `shiten docs-audit` | Valida sincronização de documentação |
