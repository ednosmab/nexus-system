...
# 🖥️ SYSTEM FIRST — Protocolo de Operações via Shiten CLI

## 🎯 Objetivo
Garantir que o agente IA usa sempre comandos `shiten` para operações de
...
| Operação | Comando Shiten | O que faz |
|---|---|---|
| Listar planos activos | `shiten plan md list` | Mostra planos em andamento |
| Ver detalhes de um plano | `shiten plan md show <id>` | Mostra conteúdo e estado |
| Actualizar status | `shiten plan md status <id> <status>` | Atualiza frontmatter + move se done |
| Marcar como concluído | `shiten plan md done <id>` | Move para done/ + publica evento |
| Detectar e arquivar | `shiten plan md lifecycle --auto` | Infere estado + valida + arquiva |
| Criar novo plano | `shiten plan md create <título>` | Cria com template padrão |

### Sessão e Feedback

| Operação | Comando Shiten | O que faz |
|---|---|---|
| Ver status do projecto | `shiten status` | Mostra saúde, maturidade, capabilities |
| Fechar sessão | `shiten feedback --outcome success` | Regista resultado da sessão |
| Feedback com notas | `shiten feedback --outcome success --notes "<notas>"` | Feedback detalhado |
| Briefing pré-sessão | `shiten briefing` | Gera contexto para próxima sessão |
...
| Corrigir automaticamente | `shiten validate --fix` | Corrige problemas detectáveis |
| Auditar saúde | `shiten audit` | Análise completa de governança |
...
5. shiten feedback --outcome success  ← fechar sessão
```

## ⚠️ Excepções (quando edição manual é aceitável)

Edição manual SÓ é permitida quando:
1. Nenhum comando `shiten` existe para a operação
