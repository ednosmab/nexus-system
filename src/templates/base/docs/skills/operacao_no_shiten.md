# Skill: Operação no Shitenno-go

Esta skill é portável — copiável para qualquer projeto que use o Shiten, sem adaptação. Ela descreve **como agir dentro desta estrutura de pastas**, não decisões específicas de nenhum projeto.
...
| `shitenno-go/profile/<projeto>.config.ts` | Aqui SIM nomes reais de pasta do projeto são esperados e corretos. |
...
| ADR de qualquer fase | **Não escreva aqui.** ADR é registrada no repositório do projeto consumidor, nunca dentro do Shiten. Se a tarefa pedir uma ADR, confirme em qual projeto ela deveria nascer antes de criar o arquivo. |
...
4. Se a tarefa tocou em regra de governança de um projeto hospedeiro
   (não do próprio Shiten), houve aprovação explícita do Tech Lead
   antes de aplicar?
