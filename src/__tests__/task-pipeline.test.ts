...
import { getEventBus, resetEventBus, type ShitennoEventType } from "../event-bus.js";
...
describe("task-pipeline", () => {
  ...
  it("subscribes to validation.completed event", () => {
    ...
    expect(bus.listenerCount("validation.completed" as ShitennoEventType)).toBeGreaterThanOrEqual(1);
  });

  it("subscribes to task.completed event", () => {
    cleanup = initializeTaskPipeline(config);
    const bus = getEventBus();
    expect(bus.listenerCount("task.completed" as ShitennoEventType)).toBeGreaterThanOrEqual(1);
  });
});
