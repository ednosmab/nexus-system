...
describe("plan detection via fs mocks", () => {
  it("finds active plan matching task ID", () => {
    ...
    const plansDir = "/nexus/governance/plans";
  ...
  it("skips done plans", () => {
    ...
    const content = mockReadFileSync("/nexus/governance/plans/plan-done.md", "utf-8") as string;
  ...
  it("returns null when plans dir does not exist", () => {
    ...
    expect(mockExistsSync("/nexus/governance/plans")).toBe(false);
  });
});
