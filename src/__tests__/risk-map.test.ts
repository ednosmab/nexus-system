...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), "nexus-risk-"));
});
...
describe("generateRiskMap", () => {
  it("returns low risk for empty project", () => {
    const result = generateRiskMap(tempDir, join(tempDir, "nexus-system"));
  ...
  it("handles non-existent project root gracefully", () => {
    const result = generateRiskMap("/nonexistent/path", "/nonexistent/nexus-system");
    expect(result.overallRisk).toBe("low");
