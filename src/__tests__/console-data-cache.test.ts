...
describe("Console Data Cache", () => {
  ...
  it("should return data on first call", () => {
    const data = getOrCollectConsoleData("/tmp", "/tmp/.shugo");
    expect(data).toBeDefined();
    expect(data.timestamp).toBeDefined();
  });

  it("should return cached data on second call within TTL", () => {
    const data1 = getOrCollectConsoleData("/tmp", "/tmp/.shugo", 5000);
    const data2 = getOrCollectConsoleData("/tmp", "/tmp/.shugo", 5000);
  ...
  it("should return fresh data after TTL expires", async () => {
    const data1 = getOrCollectConsoleData("/tmp", "/tmp/.shugo", 1);
    await new Promise((r) => setTimeout(r, 10));
    const data2 = getOrCollectConsoleData("/tmp", "/tmp/.shugo", 1);
    expect(data1).toBeDefined();
    expect(data2).toBeDefined();
  });

  it("should use different cache keys for different project roots", () => {
    const data1 = getOrCollectConsoleData("/tmp/project1", "/tmp/project1/.shugo");
    const data2 = getOrCollectConsoleData("/tmp/project2", "/tmp/project2/.shugo");
    expect(data1.projectRoot).toBe("/tmp/project1");
    expect(data2.projectRoot).toBe("/tmp/project2");
  });

  it("should clear all cache entries", () => {
    getOrCollectConsoleData("/tmp", "/tmp/.shugo");
    clearConsoleDataCache();
    const data = getOrCollectConsoleData("/tmp", "/tmp/.shugo");
    expect(data).toBeDefined();
  });
