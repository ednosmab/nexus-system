...
describe("readManifest", () => {
  it("reads valid manifest", () => {
    ...
    expect(readManifest("/nexus")).toEqual(manifest);
  });

  it("returns null when file does not exist", () => {
    mockExistsSync.mockReturnValue(false);
    expect(readManifest("/nexus")).toBeNull();
  });
...
describe("scanTemplateHashes", () => {
  it("returns empty object for empty directory", () => {
    ...
    expect(scanTemplateHashes("/nexus")).toEqual({});
  });
  ...
  it("skips node_modules and .git", () => {
    ...
    const hashes = scanTemplateHashes("/nexus");
...
describe("createManifest", () => {
  it("creates manifest with correct fields", () => {
    mockReaddirSync.mockReturnValue([]);
    const manifest = createManifest("1.2.3", "/nexus", ["core", "ai"], 80);
...
describe("updateManifest", () => {
  it("creates new manifest when no current exists", () => {
    mockReaddirSync.mockReturnValue([]);
    const manifest = updateManifest(null, "2.0.0", "/nexus", ["core"], 70);
  ...
  it("merges hashes from current manifest", () => {
    ...
    const updated = updateManifest(current, "2.0.0", "/nexus", ["core", "ai"], 75);
